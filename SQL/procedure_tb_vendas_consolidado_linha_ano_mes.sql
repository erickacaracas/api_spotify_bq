CREATE OR REPLACE PROCEDURE `data-engineering-3254.trusted_vendas.sp_tb_vendas_consolidado_linha_ano_mes`()
BEGIN
  -- Declaração das variáveis
  DECLARE VAR_PROJETO STRING DEFAULT 'data-engineering-3254';
  DECLARE VAR_DATASET_TRUSTED STRING DEFAULT 'trusted_vendas';
  DECLARE VAR_TABELA_DESTINO STRING DEFAULT 'tb_vendas_consolidado_linha_ano_mes';
  DECLARE VAR_TABELA_ORIGEM STRING DEFAULT 'tb_vendas';

  -- Criação da tabela temporária temp_consolidado_linha_ano_mes
  EXECUTE IMMEDIATE  """
  CREATE OR REPLACE TEMP TABLE temp_consolidado_linha_ano_mes AS
  SELECT
    LINHA,
  ...
  EXECUTE IMMEDIATE  """
    ...
    SELECT 
        ...
        MES, 
        TOTAL_VENDAS
    FROM temp_consolidado_linha_ano_mes
  """;
END;
